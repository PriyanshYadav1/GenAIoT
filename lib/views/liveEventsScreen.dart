...
class _LiveEventsScreenState extends State<LiveEventsScreen> {
  ...
  Future<void> initializeGaugeData() async {
    ...
    await liveEvents();
    await HeartbeatWidget(assetId: widget.assetId);
    await LifeCycle(assetId: widget.assetId);
    ...
    try {
      ...
      if (liveData != null && liveData["data"] != null) {
        ...
        for (var item in data) {
          var edgeTelemetryModelData = await getFromSqllite(
              "telemetry_model", "data_model" + item['asset_model_short_code']);
          var widgetModelData = await getFromSqlListing(
              "Latest Telemetry", "widget" + item['asset_model_short_code']);
          ...
          final dateTime = DateTime.fromMillisecondsSinceEpoch(
              (timestamp * 1000).toInt(), isUtc: isUtc);
          final dateFormat = DateFormat('yyyy-MM-dd   HH:mm:ss');
          final formattedDate = dateFormat.format(dateTime);
          final Map<String, dynamic> itemData = item['data'] as Map<
              String,
              dynamic>? ?? {};
          ...
          for (var entry in itemData.entries) {
            ...
            for (var metric in metrics.entries) {
              var dataAll = await getValueByPropertyJsonKey(
                  "telemetry_model", edgeTelemetryModelData,
                  metric.key.toString(), "property_json_key", widgetModelData,
                  "Latest Telemetry");
              if (dataAll != null) {
                transformedData.add({
                  'devId': devId,
    ...
    try {
      ...
      if (liveData != null && liveData["data"] != null) {
        ...
        for (var item in data) {
          ...
          var widgetModelData = await getFromSqlListing(
              "Recent Telemetry", "widget" + item['asset_model_short_code']);
          final double timestamp = (item['ts'] as num?)?.toDouble() ?? 0.0;
          final dateTime = DateTime.fromMillisecondsSinceEpoch(
              (timestamp * 1000).toInt(), isUtc: isUtc);
          final dateFormat = DateFormat('yyyy-MM-dd   HH:mm:ss');
            ...
            for (var metric in metrics.entries) {
              ...
              if (dataAll != null) {
                transformedData.add({
                  'devId': devId,
                  'name': metric.key,
                  'value': dataAll["property_data_type"] == "float"
                      ? double.parse(metric.value.toString()).toStringAsFixed(2)
                ...
                if (metric.value is num) {
                  ...
                  if (stringValue.isEmpty || stringValue == "N/A" ||
                      stringValue == "null") {
  ...
  Future<void> liveEvents() async {
    try {
      ...
      if (liveData != null && liveData["data"] != null) {
        ...
        for (var item in data) {
          var edgeTelemetryModelData = await getFromSqllite(
              "event_model", "data_model" + item['asset_model_short_code']);
          ...
          for (var entry in itemData.entries) {
            print("ENNNTRRYYYYYYYYYYYY " + entry.toString() + "   key  " +
                entry.key.toString() + " value " +
                entry.value["ecode"].toString());
            final devId = entry.key;
            final metrics = entry.value as Map<String, dynamic>;

            for (var metric in metrics.entries) {
              print(entry.value["eseverity"].toString() +
                  "entry.value[entry.value[");
              var dataAll = await getValueByPropertyJsonKeydata(
                  "live", edgeTelemetryModelData,
                  entry.value["ecode"].toString(), "event_code",
                  widgetModelData, "");
              ...
              transformedData.add({
                ...
                'property_display_name': entry.value["emessage"] != null ? entry
                    .value["emessage"].toString() : dataAll != null
                    ? dataAll["event_message"]
                    : "N/A",
                'property_unit': entry.value["eseverity"] != null ? entry
                    .value["eseverity"].toString() : dataAll != null
                    ? dataAll["event_severity"]
                    : "N/A"
              });
            }
          }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Error'),
          content: Text(
              'An error occurred while fetching data ${e} ${stackTrace}'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          SizedBox(width: 10),
          // Add some spacing between the SizedBox and the right edge
        ],
...
////////////////////////////////////////////////////////////////////////////////////////
  Widget _buildSelectedView() {
    final Map<String, List<FlSpot>> graphData = _prepareGraphData(
        recentTelemetryData);
    switch (_selectedIndex) {
      ...
      case 3:
      // return _assetHeartbeat(heartbeatData);
              ...
              child: Column(
                children: [
                  ...
                  Column(
                    children: [
                      Text(
                        formateData(gaugeData["property_data_type"],
                            gaugeData["value"]),
                        style: TextStyle(
                          fontSize: 48,
  ...
  Map<String, List<FlSpot>> _prepareGraphData(List<dynamic> telemetryData) {
    ...
    for (var item in telemetryData) {
      ...
      if (item["property_data_type"] == "float" ||
          item["property_data_type"] == "integer") {
        int index = graphData[displayName]!
            .length; // Use the length as the x-axis
        graphData[displayName]!.add(
            FlSpot(index.toDouble(), dataFinalVal as double));
      } else {
        graphData[displayName]!.add(FlSpot(0, 0));
      }
      // Format the timestamp for the x-axis label
      String xLabel = DateFormat('yyyy-MM-dd HH:mm:ss').format(
          item['timestamp']);
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final gaugeData = {
          ...
          "ts": latestTimestamp.isNotEmpty ? latestTimestamp : DateTime.now()
              .toString(),
          "property_unit": Unit,
        ...
        final double yInterval = getDynamicInterval(
            getDataMin(dataPoints), getDataMax(dataPoints),
            1); // 6 labels on y-axis
                    ...
                    children: [
                      Text(
                        gaugeData["property_data_type"] == "integer"
                            ? (dataPoints.isNotEmpty ? dataPoints.last.y
                            .toStringAsFixed(0) : 'N/A')
                            : gaugeData["property_data_type"] == "float"
                            ? (dataPoints.isNotEmpty ? dataPoints.last.y
                            .toString() : 'N/A')
                            : gaugeData["property_data_type"] == "enum"
                            ? formateData(
                            gaugeData["property_data_type"], gaugeData["value"])
                            : gaugeData["property_data_type"] == "boolean"
                            ? formateData(gaugeData["property_data_type"],
                            gaugeData["value"])
                            : gaugeData["property_data_type"] != null
                            ? "jh"
                            : "",
...
///////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////
  dynamic getValueByPropertyJsonKey(String type, List<dynamic> widget_data,
      String propertyJsonKey, String keyName, List widgetModelData,
      ...
      for (var asset_data in widgetModelData) {
        ...
        if (asset_data["purpose"] == TelemetryName) {
          if (asset[keyName] == propertyJsonKey) {
            asset_d =
                asset; // Return the whole asset or modify this to return a specific property if needed
          }
        }
...
///////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////
  dynamic getValueByPropertyJsonKeydata(String type, List<dynamic> widget_data,
      String propertyJsonKey, String keyName, List widgetModelData,
      String TelemetryName) {
    var asset_d = null;
        ...
        title: Row(
          ...
          children: [
            ...
            if(data["property_unit"] == "Warning")...[
              ...
              if(data["property_unit"] == "Info")...[
                Icon(
                  Icons.info, color: Colors.blue,
                  size: 24,
                )
              ] else
                if(data["property_unit"] == "Critical")...[
                  Icon(
                    Icons.warning_outlined, color: Colors.red,
                    size: 24,
                  )
                ] else
                  if(data["property_unit"] == "Error")...[
                    Icon(
                      Icons.warning_outlined, color: Colors.red,
                      size: 24,
                    )
                  ] else
                    if(data["property_unit"] == "Information")...[
                      Icon(
                        Icons.info, color: Colors.blue,
                        size: 24,
                      )
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildRichTextRow(
                            "Message:", data['property_display_name'] ?? 'N/A'),
                        // _buildRichTextRow("Message:", data['ack'] ?? 'N/A'),
                        _buildRichTextRow(
                            "Child Device:", data['devId'] ?? 'N/A'),
          ...
          Expanded(
            child: Align(
              ...
              child: Text(
                ...
                style: TextStyle(fontSize: 13, color: Colors.grey[900]),
                textAlign: TextAlign
                    .left, // Align text to the left within the box
              ),
            ),
          ),
        ],
